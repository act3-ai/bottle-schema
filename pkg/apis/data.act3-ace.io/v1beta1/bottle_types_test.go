...
# Each bottle part may also have "part labels".  Those can be added with "ace-dt bottle commit --label" or directly in the .labels.yaml files.
`
...
func TestBottle_Validate(t *testing.T) {
	...
	err := bottle.Validate()
	assert.Error(err)
	assert.Equal(err.Error(), "metrics: (1: (value: must be a floating point number.).).")
}
